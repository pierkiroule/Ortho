...
type Draft = { selectedIds: string[]; priorityIds: string[]; weather: WeatherOption | null }
const emptyDraft = (): Draft => ({ selectedIds: [], priorityIds: [], weather: null })
...
function App() {
  ...
  const summary = savedEntry ?? (draft.weather ? createSummary(selectedCards, priorityCards, draft.weather, mode) : null)
  ...
  function switchMode(nextMode: Perspective) { setMode(nextMode); setSavedEntry(null); goTo(1) }
  ...
  function onTouchEnd(event: React.TouchEvent<HTMLElement>) {
    ...
    const next = delta > 0 ? Math.min(5, screen + 1) : Math.max(0, screen - 1)
    if (next === 2 && draft.selectedIds.length === 0) return notify('Choisis au moins une carte avant de continuer')
    if (next === 3 && draft.priorityIds.length === 0) return notify('Choisis au moins une priorité')
    if (next === 4 && !draft.weather) return notify('Choisis une météo avant la synthèse')
    goTo(next as Screen)
  }
  ...
  function showSummary() {
    ...
    const entry = createSummary(selectedCards, priorityCards, draft.weather, mode)
    setSavedEntry(entry)
  ...
  async function downloadPng(target: HTMLDivElement | null, filename: string) {
    ...
    const dataUrl = await toPng(target, { backgroundColor: '#f5f7fb', pixelRatio: 2 })
  ...
  return (
    <main className={`app screen-${screen}`} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      {screen === 1 && <section className="screen"><StepIntro title="Balance moteur / freins" text={mode === 'patient' ? 'Swipe ou touche les cartes : ce qui t’aide à avancer et ce qui freine ton traitement.' : 'Swipe ou touchez les cartes : ce qui aide votre enfant et ce qui semble freiner son traitement.'} mode={mode} /><CardGroup title="🌟 Moteurs" group="resource" mode={mode} selectedIds={draft.selectedIds} onToggle={toggleSelected} /><CardGroup title="🌧️ Freins" group="difficulty" mode={mode} selectedIds={draft.selectedIds} onToggle={toggleSelected} /><button className="btn btn-primary btn-sticky" type="button" disabled={draft.selectedIds.length === 0} onClick={() => goTo(2)}>Continuer</button></section>}
      {screen === 2 && <section className="screen"><StepIntro title="Météo globale" text="Choisis les 3 cartes prioritaires puis la météo émotionnelle du moment." mode={mode} /><p className="priority-counter">{draft.priorityIds.length} / 3 priorités choisies</p><div className="card-grid">{selectedCards.map((card) => <CardButton key={card.id} card={{ ...card, label: mode === 'parent' ? card.parentLabel : card.label }} selected={draft.priorityIds.includes(card.id)} priority={draft.priorityIds.includes(card.id)} onClick={() => togglePriority(card.id)} />)}</div><div className="weather-grid weather-inline">{weatherOptions.map((option) => <button key={option.id} className={`weather-btn ${draft.weather?.id === option.id ? 'selected' : ''}`} type="button" onClick={() => updateDraft({ weather: option })}><span className="weather-emoji">{option.emoji}</span><span className="weather-label">{option.label}</span></button>)}</div><button className="btn btn-primary btn-sticky" type="button" disabled={draft.priorityIds.length === 0 || !draft.weather} onClick={showSummary}>Générer la synthèse</button></section>}
      {screen === 3 && summary && <section className="screen"><SummaryCard refEl={resultRef} entry={summary} selectedCards={selectedCards} priorityCards={priorityCards} /><div className="result-actions"><button className="btn btn-primary" type="button" onClick={() => goTo(4)}>Voir l’évolution</button><button className="btn btn-secondary" type="button" onClick={() => switchMode(mode === 'patient' ? 'parent' : 'patient')}>Compléter le parcours {mode === 'patient' ? 'parents' : 'patient'}</button></div></section>}
      {screen === 4 && <section className="screen"><HistoryDashboard refEl={historyRef} entries={history} patient={latestPatient} parent={latestParent} /><div className="result-actions"><button className="btn btn-primary" type="button" onClick={() => goTo(5)}>Partager</button><button className="btn btn-secondary" type="button" onClick={() => switchMode('patient')}>Nouvel EchoMood patient</button><button className="btn btn-secondary" type="button" onClick={() => switchMode('parent')}>Nouvel EchoMood parents</button></div></section>}
      {screen === 5 && <section className="screen"><ShareView reportRef={reportRef} entries={history} patient={latestPatient} parent={latestParent} /><div className="result-actions no-print"><button className="btn btn-primary" type="button" disabled={history.length === 0} onClick={printPdfReport}>📄 Rapport PDF horodaté</button><button className="btn btn-secondary" type="button" disabled={!!summary === false} onClick={copySynthesis}>📋 Copier la synthèse active</button><button className="btn btn-secondary" type="button" disabled={!summary} onClick={() => summary && downloadXml(createSummaryXml(summary), 'echomood-synthese.xml')}>🧾 XML synthèse</button><button className="btn btn-secondary" type="button" disabled={history.length === 0} onClick={() => downloadXml(createHistoryXml(history), 'echomood-evolution.xml')}>🧾 XML évolution</button><button className="btn btn-secondary" type="button" disabled={history.length === 0} onClick={() => downloadPng(reportRef.current, 'echomood-rapport.png')}>🖼️ PNG rapport</button><button className="btn btn-ghost" type="button" onClick={reset}>🔄 Revenir à l’accueil</button></div></section>}
...
function Home({ mode, onStart, onHistory }: { mode: Perspective; onStart: (mode: Perspective) => void; onHistory: () => void }) { return <section className="screen home-screen"><div className="hero"><EffervescentBubbles variant="hero" /><span className="hero-badge">Swipe horizontal • parcours en 5 étapes</span><h1 className="hero-title" aria-label="EchoMood Ortho"><span className="hero-title-word">EchoMood</span><span className="hero-title-mark">•°</span><span className="hero-title-accent">Ortho</span></h1><p className="hero-subtitle">Une UI plus aérée, façon carte iOS, pour croiser le vécu patient et parents.</p><div className="journey-map"><Progress step={0} /></div><div className="mode-grid" aria-label="Choisir un parcours"><button className={`mode-card ${mode === 'patient' ? 'selected' : ''}`} type="button" onClick={() => onStart('patient')}><strong>🧑 Mode patient</strong><span>« Comment je le vis ? »</span></button><button className={`mode-card ${mode === 'parent' ? 'selected' : ''}`} type="button" onClick={() => onStart('parent')}><strong>👪 Mode parents</strong><span>« Comment votre enfant le vit ? »</span></button></div><button className="btn btn-secondary" type="button" onClick={onHistory}>📈 Voir synthèse & évolution</button></div></section> }
