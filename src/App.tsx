...
function SummaryCard({ refEl, entry, selectedCards, priorityCards }: { refEl: RefObject<HTMLDivElement | null>; entry: EchoMoodEntry; selectedCards: typeof cards; priorityCards: typeof cards }) {
  ...
  return <div ref={refEl} className="result-capture"><span className="mode-pill">{isParent ? 'Mode Parent' : 'Mode Jeune'}</span><h2 className="result-title">{isParent ? 'Voici l’EchoMood Parent' : 'Voici ton EchoMood aujourd’hui'}</h2><p className="result-date">{new Date(entry.createdAt).toLocaleString('fr-FR', { dateStyle: 'full', timeStyle: 'short' })}</p><article className="result-section rosace-main-card"><EchoMoodRosace echoMood={compact} variant="single" showLegend animated={false} /></article><ResultSection title={isParent ? '🌦️ Météo perçue' : '🌦️ Météo'} cards={[{ ...entry.weather, parentLabel: entry.weather.label, group: 'resource' }]} /><ResultSection title={isParent ? '🎈 Charge perçue' : '🎈 Charge'} cards={entry.treatmentWeight ? [{ ...entry.treatmentWeight, parentLabel: entry.treatmentWeight.label, group: 'resource' }] : []} /><ResultSection title={isParent ? '⭐ Carte du jour perçue' : '⭐ Carte du jour'} cards={priorityCards} priority /><ResultSection title={isParent ? '🌟 Ressources sélectionnées' : '🌟 Ressources sélectionnées'} cards={selectedCards.filter((card) => card.group === 'resource')} /><ResultSection title={isParent ? '🌧️ Difficultés sélectionnées' : '🌧️ Difficultés sélectionnées'} cards={selectedCards.filter((card) => card.group === 'difficulty')} /><article className="result-section synthesis-box"><h3>🧬 Code EchoMood</h3><p>{encodeEchoMood(compact)}</p></article><article className="result-section question-box"><h3>💬 Question d’ouverture</h3><p>{entry.suggestedQuestion}</p></article></div>
}
