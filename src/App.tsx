...
function App() {
  ...
  return (
    <main className={`app screen-${screen}`} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      {screen === 1 && <section className="screen"><StepIntro title="MétéoMood" text={mode === 'patient' ? 'Quelle est la météo de ton humeur globale aujourd’hui ?' : 'Quelle est, selon vous, la météo de l’humeur globale de votre enfant aujourd’hui ?'} mode={mode} /><div className="weather-grid weather-first">{weatherOptions.map((option) => <button key={option.id} className={`weather-btn ${draft.weather?.id === option.id ? 'selected' : ''}`} type="button" onClick={() => updateDraft({ weather: option })}><span className="weather-emoji">{option.emoji}</span><span className="weather-label">{option.label}</span></button>)}</div><button className="btn btn-primary btn-sticky" type="button" disabled={!draft.weather} onClick={() => goTo(2)}>Continuer</button></section>}
      {screen === 2 && <section className="screen"><StepIntro title="Balance moteur / freins" text={mode === 'patient' ? 'Choisis ce qui t’aide à avancer et ce qui freine ton traitement, puis marque jusqu’à 3 priorités.' : 'Choisissez ce qui aide votre enfant et ce qui semble freiner son traitement, puis marquez jusqu’à 3 priorités.'} mode={mode} /><CardGroup title="🌟 Moteurs" group="resource" mode={mode} selectedIds={draft.selectedIds} onToggle={toggleSelected} /><CardGroup title="🌧️ Freins" group="difficulty" mode={mode} selectedIds={draft.selectedIds} onToggle={toggleSelected} /><p className="priority-counter">{draft.priorityIds.length} / 3 priorités choisies</p><div className="card-grid priority-grid">{selectedCards.map((card) => <CardButton key={card.id} card={{ ...card, label: mode === 'parent' ? card.parentLabel : card.label }} selected={draft.priorityIds.includes(card.id)} priority={draft.priorityIds.includes(card.id)} onClick={() => togglePriority(card.id)} />)}</div><button className="btn btn-primary btn-sticky" type="button" disabled={draft.priorityIds.length === 0} onClick={showSummary}>Générer la synthèse</button></section>}
